...
export function LoadingScreen() {
  ...
  useEffect(() => {
    ...
    const progressTimer = setInterval(() => {
      setProgress((prev) => {
        ...
        return Math.min(prev + (prev < 40 ? 3 : prev < 70 ? 2 : 1), 100);
      });
    }, 50);
    const phaseTimers = [
  ...
  useEffect(() => {
    ...
    interface P { angle: number; radius: number; targetRadius: number; size: number; speed: number; opacity: number; char?: string; }
      ...
      <div className="flex flex-col items-center gap-6">
        <div className="relative w-72 h-72 flex items-center justify-center">
          ...
          <div className="relative z-10 text-center">
            <span className={`text-4xl font-black text-gold-gradient-animated transition-all duration-1000 ${phase === 'revealing' ? 'scale-110' : ''}`}>
              {phase === 'revealing' ? '命' : '占'}
            </span>
          </div>
        </div>
        ...
        <div className="text-center space-y-4 min-w-[280px]">
          <p key={messageIndex} className="text-gold-400/90 text-lg tracking-[0.15em] animate-fade-in">
            {MESSAGES[messageIndex]}...
          </p>
          <div className="relative w-48 h-[2px] mx-auto bg-navy-800 rounded-full overflow-hidden">
            <div className="absolute inset-y-0 left-0 bg-gradient-to-r from-gold-600 via-gold-400 to-gold-600 rounded-full transition-all duration-200" style={{ width: `${progress}%` }} />
            <div className="absolute inset-y-0 left-0 bg-gold-300/50 rounded-full blur-sm transition-all duration-200" style={{ width: `${progress}%` }} />
          </div>
        </div>
      </div>
    </div>
