    ...
    <div className="flex flex-col items-center justify-center min-h-screen px-6 py-12 text-center">
      ...
      <div className="relative z-10 flex flex-col items-center gap-14">
          ...
          <div className="relative w-56 h-56 flex items-center justify-center">
            <svg viewBox="0 0 240 240" className="absolute w-72 h-72 animate-slow-spin">
              ...
              {Array.from({ length: 8 }).map((_, i) => {
                ...
                return <circle key={i} cx={120 + Math.cos(angle) * 110} cy={120 + Math.sin(angle) * 110} r="2" fill="#d4a017" opacity="0.6" />;
              })}
            </svg>
            <svg viewBox="0 0 200 200" className="absolute w-60 h-60 animate-slow-spin-reverse">
              ...
              {Array.from({ length: 12 }).map((_, i) => {
                ...
                return <circle key={i} cx={100 + Math.cos(angle) * 85} cy={100 + Math.sin(angle) * 85} r="1.2" fill="#f9db66" opacity="0.4" />;
              })}
            </svg>
        ...
        <div className="space-y-5">
          <h1 className={`text-5xl md:text-6xl font-black tracking-[0.4em] text-gold-gradient-animated ${mounted ? 'animate-fade-in-up stagger-2 opacity-0' : 'opacity-0'}`}>
            四柱推命
          </h1>
          <p className={`text-xl text-navy-200/80 tracking-[0.3em] font-medium ${mounted ? 'animate-fade-in-up stagger-3 opacity-0' : 'opacity-0'}`}>
            運 命 鑑 定
          </p>
          <div className={`w-32 h-px mx-auto animate-glow-line ${mounted ? 'animate-fade-in stagger-3 opacity-0' : 'opacity-0'}`} />
          <p className={`text-sm text-navy-300/70 leading-loose max-w-xs mx-auto tracking-wider ${mounted ? 'animate-fade-in-up stagger-4 opacity-0' : 'opacity-0'}`}>
            生年月日と出生時刻から<br />
            あなたの本質・恋愛・仕事・運勢を<br />
            読み解きます
          </p>
        ...
        <button
          onClick={onStart}
          className={`group relative mt-2 ${mounted ? 'animate-fade-in-up stagger-5 opacity-0' : 'opacity-0'}`}
        >
